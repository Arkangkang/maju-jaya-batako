...
export default function InvoiceData() {
  ...
  const handleToggleStatus = async (inv) => {
    ...
    // Paid → Unpaid: just toggle, no proof needed
    ...
    if (!error) {
      ...
      if (selectedInvoice?.id === inv.id) {
        setSelectedInvoice({ ...selectedInvoice, status: "unpaid", payment_proof_url: null });
      }
    }
  };
  ...
  const handlePaymentProofSubmit = async () => {
    if (!proofFile || !proofInvoice) return;
    setProofLoading(true);
    try {
      const ext = proofFile.name.split(".").pop();
      const fileName = `${proofInvoice.id}_${Date.now()}.${ext}`;
      ...
      setProofPreview(null);
      fetchInvoices();
      if (selectedInvoice?.id === proofInvoice.id) {
        setSelectedInvoice({
          ...selectedInvoice,
          status: "paid",
          payment_proof_url: urlData.publicUrl,
      ...
      toast.error("Gagal: " + (err.message || "Upload error"));
    } finally {
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "24px" }}>
              ...
              {/* Bukti Pembayaran */}
              {selectedInvoice.status === "paid" && selectedInvoice.payment_proof_url && (
                <div
                  style={{
                    marginBottom: "24px",
                    padding: "16px",
                ...
                >
                  ...
                  >
                    <CheckCircle size={14} /> Bukti Pembayaran
                  </p>
                  {selectedInvoice.payment_proof_url.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    <a
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "24px" }}>
              ...
              >
                Upload bukti pembayaran (foto atau PDF) untuk menandai invoice
                ini sebagai <strong style={{ color: "#16a34a" }}>Lunas</strong>.
              </p>
              ...
              <label
                style={{
                  ...
                  borderRadius: "12px",
                  border: `2px dashed ${
                    proofFile ? "#22c55e" : "var(--color-border)"
                  }`,
                  backgroundColor: proofFile
                    ? "#f0fdf4"
                    : "var(--color-bg-secondary)",
                  cursor: "pointer",
                  transition: "all 0.2s",
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  padding: "12px",
                  borderRadius: "8px",
                  backgroundColor: !proofFile ? "#9ca3af" : "#22c55e",
                  color: "white",
                  fontWeight: "700",
                  border: "none",
                  cursor:
                    !proofFile || proofLoading ? "not-allowed" : "pointer",
              ...
              >
                <CheckCircle size={16} />
                {proofLoading ? "Mengupload..." : "Konfirmasi Lunas"}
              </button>
            </div>
          </div>
        </div>
